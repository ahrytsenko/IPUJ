...
public class RandomMosaicWalk3 {
    ...
    public static void main(String[] args) {
        ...
        while (Mosaic.isOpen()) {
            currentRow = (int)(Math.random()*ROWS); //Select a random Row
            currentColumn = (int)(Math.random()*COLUMNS); //Select a random Column
            copyColorToNeighbors(currentRow, currentColumn);
            Mosaic.delay(1);
        }
    }  // end main
    ...
    /**
     * Copy the color of one square to neighbors (above, below, left and right).
     * Precondition:   no
     * Postcondition:  Neighbors will be the same color as selected square
     ...
     *      from 0 at the left
     */
    static void copyColorToNeighbors(int rowNum, int colNum) {
        Color color = Mosaic.getColor(rowNum, colNum);
        
        if (rowNum > 0) {Mosaic.setColor(rowNum-1, colNum, color);}
        if (colNum > 0) {Mosaic.setColor(rowNum, colNum-1, color);}
        if (rowNum < ROWS-1) {Mosaic.setColor(rowNum+1, colNum, color);}
        if (colNum < COLUMNS-1) {Mosaic.setColor(rowNum, colNum+1, color);}
        
    }  // end changeToRandomColor

    /**
     * Changes one square to a new random color.
